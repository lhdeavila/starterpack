#!/bin/bash
# author: eleAche
# description: recovers txt files, filters duplicates, and organizes for later compression.
...
sudo find / -iname '*.txt' -exec sha1sum {} \; >> $dashboard
remover_duplicados=$(cat $dashboard | awk '{print $1}' | sort -u | uniq)

# object processing
formato_de_almacenamiento() {
  local fichero="$1"
  ruta_de_fichero=$(grep $fichero $dashboard | head -1 | awk '{print $2,$NF}')
...
for hash_fichero in $remover_duplicados
  do
    ...
    formato_de_almacenamiento $hash_fichero
  done 2>/dev/null
